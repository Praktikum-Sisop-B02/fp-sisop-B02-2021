...
int create_user(char *username)
{
    ...
    while (i--)
    {
        if (!strcmp(user_list[i].username, username))
        {
            return 1;
        };
    }
...
int run_db_command()
{
    ...
    */
    ...
    {
        char tmp[str_size];
        char *subcmd = uppercase(get_string_between(query, "DELETE ", " "));
        printf("\t[%s]\n", subcmd);

        if (!strcmp(subcmd, "FROM"))
        {
            char *tableName = lowercase(get_string_between(query, "FROM ", " "));
            //No Where Query
            if (tableName == NULL)
            {
                tableName = lowercase(get_string_between(query, "FROM ", ";"));
                printf("\t   --  Table Name: {%s}\n", tableName);
                return 1;
            }
            //Where Query Exists
            else
            {
                ...
                {
                    char *condition = get_string_between(query, "WHERE ", ";");
                    char *col = get_string_between(condition, NULL, "=");
                    char *val = remove_single_quotes(get_string_between(condition, "=", NULL));
                    printf("\t   [%s]\n", subcmd);
                    printf("\t      --  Column: {%s}\n", col);
                    printf("\t      --  Value: {%s}\n", val);
                }
    ...
    */
    ...
    {
        char tmp[str_size];
        char *subcmd = uppercase(get_string_between(query, "DROP ", " "));
        printf("\t[%s]\n", subcmd);
        ...
        {
            char *columnName = lowercase(get_string_between(query, "COLUMN ", " "));
    ...
    */
    ...
    {
        char tmp[str_size];
        char *subcmd = uppercase(get_string_between(query, "INSERT ", " "));
        printf("\t[%s]\n", subcmd);

        if (!strcmp(subcmd, "INTO"))
        {
            char *tableName = lowercase(get_string_between(query, "INTO ", " "));
            sprintf(tmp, "%s (", tableName);
            printf("\t   --  Table Name: {%s}\n", tableName);

            char *values = lowercase(get_string_between(query, tmp, ");"));

            char *p;
            char *val = remove_foward_and_trailing_whitespace(strtok_r(values, ",", &p));
            while (val != NULL)
            {
                val = remove_single_quotes(val);
                printf("\t      --  {%s}\n", val);
                val = remove_foward_and_trailing_whitespace(strtok_r(NULL, ",", &p));
            }
            return 1;
        }
    ...
    */
    else if (!strcmp(cmd, "SELECT"))
    {
        ...
        char *columns = lowercase(get_string_between(query, "SELECT ", " FROM"));
        ...
        else
        {
            ...
            sprintf(tmp, "%s ", tableName);
            char *subcmd = uppercase(get_string_between(query, tmp, " "));
    ...
    */
    ...
    {
        char tmp[str_size];
        char *tableName = lowercase(get_string_between(query, "UPDATE ", " "));
        printf("\t--  Table Name: {%s}\n", tableName);
        ...
        {
            char *toChange = get_string_between(query, "SET ", " ");

            //No Where Query
            if (toChange == NULL)
            {
                toChange = lowercase(get_string_between(query, "SET ", ";"));
                char *col = get_string_between(toChange, NULL, "=");
                char *val = remove_single_quotes(get_string_between(toChange, "=", NULL));
            ...
            else
            {
                ...
                char *col1 = get_string_between(toChange, NULL, "=");
                char *val1 = remove_single_quotes(get_string_between(toChange, "=", NULL));
                ...
                {
                    char *condition = get_string_between(query, "WHERE ", ";");
                    char *col2 = get_string_between(condition, NULL, "=");
                    char *val2 = remove_single_quotes(get_string_between(condition, "=", NULL));
